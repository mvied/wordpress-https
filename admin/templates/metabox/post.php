...
<?php } ?>
	<label<?php echo ( isset($parent_force_ssl_children)  ? ' title="' . __('This post\'s parent page \'','wordpress-https') . $parent_force_ssl_children->post_title . __('\' has \'Secure child posts\' enabled.','wordpress-https') . '"' : '' ); ?>><input type="checkbox" value="1" name="force_ssl" <?php echo ( $force_ssl  ? ' checked="checked"' : '' ); ?><?php echo ( isset($parent_force_ssl_children)  ? ' disabled="disabled="' : '' ); ?> /> <?php _e('Secure post','wordpress-https'); ?></label>
</div>
